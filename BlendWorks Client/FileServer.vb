...
Public Class WebServer
    ...
    Private Sub sendResponse(clientSocket As Socket, bContent As Byte(), responseCode As String, contentType As String)
        Try
            Dim bHeader As Byte() = charEncoder.GetBytes("HTTP/1.1 " & responseCode & vbCr & vbLf & "Server: setagon WebServer" & vbCr & vbLf & "Content-Length: " & bContent.Length.ToString() & vbCr & vbLf & "Connection: close" & vbCr & vbLf & "Content-Type: " & contentType & vbCr & vbLf & vbCr & vbLf)
    ...
    Public Sub New(ByVal rootPath As String)
        ...
        Task.Run( _
            Sub()
                ...
                http.Prefixes.Add("http://+:" + CLIENT_WEB_SERVER_PORT.ToString + "/")
                http.Start()
